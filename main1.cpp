...
static void spartos_testas_list(const std::string& path, Mode m) {
    ...
    for (auto it = L.begin(); it != L.end(); ) {
        ...
        auto cur = it++;
        if (v <= 5.0) varg.splice(varg.end(), L, cur);
        else          kiet.splice(kiet.end(), L, cur);
    }
    auto t_split = now() - t0;
    t0 = now();
    std::vector<Student> vv(varg.begin(), varg.end());
    std::vector<Student> kk(kiet.begin(), kiet.end());
    rasyti_grupe("vargsiukai.txt", vv, m);
    rasyti_grupe("kietiakiai.txt", kk, m);
    auto t_write = now() - t0;
    ...
    std::cout << vec.size() << " irasu rusiavimo laikas (list.sort): "
              << std::fixed << std::setprecision(6) << ms(t_sort) << " ms\n";
    std::cout << vec.size() << " irasu dalijimo i dvi grupes laikas (splice): "
              << ms(t_split) << " ms\n";
    std::cout << vec.size() << " irasu irasymo i failus laikas: "
              << ms(t_write) << " ms\n";
